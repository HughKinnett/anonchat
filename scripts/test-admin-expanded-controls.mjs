...
assert.match(rules, /function accountNotSuspended\(uid\)/, "Firestore rules define an active suspension gate");
assert.match(rules, /accountModeration\/\$\(uid\)/, "the suspension gate reads the account moderation record");
assert.match(rules, /function activeUser\(\)[\s\S]{0,300}accountNotSuspended\(request\.auth\.uid\)/, "suspended users cannot perform normal active-user writes");
...
assert.match(rules, /match \/posts\/\{postId\}[\s\S]{0,250}allow create: if[^;]*featureEnabled\('postingEnabled'\)/, "timeline posting obeys the posting switch");
