...
export const fixedResultSummary = result => `PROCESSOR_RESULT inspected=${result.inspected} processed=${result.processed} failed=${result.failed} skipped=${result.skipped} cleaned=${result.cleaned}`;

const createProductionRuntime = async projectId => {
  const app = initializeApp({ credential: applicationDefault(), projectId });
  return {
    adapter: new FirestoreModerationDeletionAdapter({ db: getFirestore(app), Timestamp, FieldPath }),
    close: () => deleteApp(app)
  };
};
...
export const main = async (argumentsList = process.argv.slice(2), dependencies = {}) => {
  ...
  try {
    return await (dependencies.processor ?? runModerationDeletionProcessor)({
      adapter: runtime.adapter,
      ownerId: (dependencies.ownerIdFactory ?? (() => `moderation-${crypto.randomUUID()}`))(),
