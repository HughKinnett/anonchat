...
onAuthStateChanged(auth, async current => { if (!current) { await exitAfterAuthLoss(); return location.replace("index.html"); } user = current; const snapshot = await getDoc(doc(db, "users", current.uid)); const data = snapshot.exists() ? snapshot.data() : {}; selectedAvatar = FREE_AVATARS.includes(data.freeAvatarId) ? data.freeAvatarId : ""; selectedCover = FREE_COVERS.includes(data.freeCoverId) ? data.freeCoverId : ""; select("avatar", selectedAvatar); select("cover", selectedCover); });
