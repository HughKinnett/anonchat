...
- Deploy the completed web feature set in one web release, not in partial feature deployments.
- Only after that complete web deployment is verified, implement the full matching feature set in the Android app.
...
- [ ] Finish Profiles + Badges, Communities, Rooms/groups, Discovery/feed controls, discussion upgrades, Premium entitlements, profile/privacy controls, notification improvements, moderation improvements, sharing improvements, and all other approved web features before deploying any of them.
- [ ] Keep each subsystem independently tested during development while holding deployment until the complete approved web feature set is ready.
- [ ] Run the full regression, security, Firestore-cost, moderation, notification, sharing, interaction-consistency, profile, admin, Premium, and compatibility test suites across the complete web release candidate.
- [ ] Merge the complete approved web feature set only after required checks are green.
- [ ] Deploy the entire completed web feature set in one web deployment and verify the deployed commit matches merged `main`.
- [ ] After the complete web deployment is verified, implement the full matching feature set in the Android app.
- [ ] Verify Android parity for Profiles + Badges, Communities, Rooms/groups, Discovery/feed controls, discussion upgrades, Premium entitlements, profile/privacy controls, notifications, moderation, sharing, and admin-only tools.
- [ ] Run the complete Android regression/parity suite after all approved Android features are present.
- [ ] Package/release Android only after the full Android feature set matches the deployed web app and all required checks pass.
