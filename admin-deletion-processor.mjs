...
const PHASE_ORDER = new Map([["claimed", 0], ["first-sweep", 1], ["profile-barrier", 2], ["second-sweep", 3], ["auth-deleted", 4]]);
...
export const runDeletionProcessor = async ({ adapter, ownerId, dryRun = false, logger = console }) => {
  ...
  if (dryRun) { for (const job of jobs) await adapter.preview(job); return result; }
  await safeHeartbeat(adapter, logger, "started");
  let markers = [];
  try { markers = await adapter.listExpiredMarkers(); } catch { safeLog(logger, "error", "MARKER_SCAN_ERROR"); }
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      if (beforePhase(phase, "auth-deleted")) {
        try { await adapter.deleteAuth(claim.targetUid, claim.token); }
        catch (error) { if (fixedErrorCode(error) !== "AUTH_NOT_FOUND") throw error; }
      }
